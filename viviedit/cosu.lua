...
local tPatterns = {
    ...
    { "^%@[%w\'_%.]+[%w]?", function(match,after, _, nLine)
        local iota_val = match:match("%@(.-[%w%s\'\"_.]+)[%(%s%)]?")
        if vivicontext.iotas[iota_val] then
            return colorMatch["iota"]
        end
...
local function autocomplete()
    ...
    local nStartPos = string.find((tContent[tCursor.y]):sub(1,tCursor.x-1), "[a-zA-Z0-9_%.:][a-zA-Z0-9_%\'.:%s]+$")
    if nStartPos then
        local comple = {}
...
--[[ +++ Popup handler +++ ]]
local update,info,help,file,error,options,exit,specialChar,openLink,openShell,openPopup, etch
...
function etch(event, ...)
    ...
    if event == "close" then
        for index,pop in pairs(tPopup) do
            if pop.name == "Etch" or pop.name == "Property Name" then
                table.remove(tPopup, index)
            end
        end
    ...
    elseif event == "genie" then
        ...
        etch("send_value", val)
    elseif event == "property" then
        local result = tArgs[1]
        local target = tArgs[2] or "egg"
        local val = {
            "property",
            target,
            result
        ...
        etch("send_value", val)
    elseif event == "property_prompt" then
        etch("close")
        openPopup({
            ["status"] = true,
            ["name"] = "Property Name",
            ...
            ["button"] = {
                { ['x']=23, ['y']=3, ["label"]="Abort", ["status"]=false, ["func"]=function() etch("close") end },
                { ['x']=29, ['y']=3, ["label"]="Yes", ["status"]=false, ["func"]=function(self) local tmp=tPopup[1].textBox[1].input etch("close") etch("save_temp", "property", tmp) end }
            }
        })
    elseif event == "compile" then
        local tempPath = tArgs[1]
        local tEvent = tArgs[2]
        local result = vivicontext.parser:new():open_file(tempPath):execute():finish().value
        fs.delete(tempPath)
    ...
    elseif event == "save_temp" then
        ...
        if f then
            ...
            f.close()
            etch("compile", tmpPath, table.unpack(tArgs))
        else
            --todo
        end
    elseif event == "send_value_message" then
        openPopup({
            ["status"] = true,
            ["name"] = "Etch",
            ["size"] = {
            ...
            ["button"] = {
                { ['x']=16, ['y']=3, ["label"]="Ok", ["status"]=false, ["func"]=function() etch("close") end }
            }
        })
    elseif event == "create" then
        openPopup({
    ...
    if #tArgs > 0 then
        ...
        if arg then
            ...
            if fs.exists(sPath) then
                ...
                if sPath:sub(#sPath-3) ~= ".hexpattern" then
                    sPath = sPath..".hexpattern"
                end
            end
        else
            table.insert(tContent, "")
