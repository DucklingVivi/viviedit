...
local function writeHighlighted(sLine)
    while #sLine > 0 do
        sLine =
            ...
            tryWrite(sLine, "^%-%-.*", commentColour) or
            tryWrite(sLine, "^%@[%w\'_%.]+[%w]?" , function(match)
            ...
            tryWrite(sLine, "^%$[%w\'_.]+[%w]?", colors.red) or
            tryWrite(sLine, "^%#[%w%s\'_.]+", colours.orange) or
            tryWrite(sLine, "^%w[%w:%s%+%-\']+%w", function(match)
...
for k, v in pairs(vivicontext.patterns) do
    table.insert(tCompleteEnv, k)
end


local function getCompletions(sLine)
    local comple = {}
    for k, v in pairs(vivicontext.patterns) do

        if string.sub(k, 1, #sLine) == sLine then
            table.insert(comple, k:sub(#sLine + 1, #k))
        end
    end
    return comple
end
...
local function complete(sLine)
    ...
    if settings.get("viviedit.autocomplete") then
        local nStartPos = string.find(sLine, "[a-zA-Z0-9_%.:][a-zA-Z0-9_%\'.:%s]+$")
        if nStartPos then
            sLine = string.sub(sLine, nStartPos)
        end
        if #sLine > 0 then
            return getCompletions(sLine)
        end
    end
    return nil
end
...
local tMenuFuncs = {
    ...
    Etch = function()
        ...
        if ok then
            local result = vivicontext.parser:new():open_file(sTempPath):execute():finish().value
            local port = peripheral.wrap("bottom")
            port.writeIota(result)
            fs.delete(sTempPath)
            sStatus = sTempPath
        else
